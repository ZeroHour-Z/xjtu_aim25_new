...
#ifndef OPENVINO_TEST_OPENVINOINFER_HPP
#define OPENVINO_TEST_OPENVINOINFER_HPP
...
public:
    ...
    void infer(cv::Mat img,int detect_color);
    ...
    void infer(cv::Mat img, float* detections){
        ...
        std::copy(output_tensor.data<float>(), output_tensor.data<float>() + 9, detections);
    }
#else
    // Minimal stubs when OpenVINO is not available
    OpenvinoInfer(std::string, std::string, std::string) {}
    void infer(cv::Mat, int) {}
#endif
...
#endif // OPENVINO_TEST_OPENVINOINFER_HPP
