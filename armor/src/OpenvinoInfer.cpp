...
OpenvinoInfer::OpenvinoInfer(std::string model_path_xml, std::string model_path_bin, std::string device){
    ...
    if (device == "CPU") {
        // 设置CPU推理性能优化
        core.set_property(device, ov::hint::num_requests(1));
        core.set_property(device, ov::hint::performance_mode(ov::hint::PerformanceMode::LATENCY));
        core.set_property(device, ov::hint::inference_precision(ov::element::f32));
        std::cout << "CPU optimizations enabled" << std::endl;
    }
...
void OpenvinoInfer::infer(cv::Mat img, int detect_color){
    objects.clear();
    tmp_objects.clear();
    ious.clear();
